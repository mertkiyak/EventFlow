...
// import { Button, Surface, Text } from "react-native-paper"; //import MaterialCommunityIcons from 'react-native-vector-icons/MaterialCommunityIcons'
...
//     if (!user || completedEvents?.includes(id)) return; // Kullanıcı yoksa işlemi durdur
//     try {
...
//             <Swipeable
//               ref={(ref) => { swipeableRefs.current[event.$id] = ref; }}
//               key={key}
//               overshootLeft={false} 
...
//               }}
//             >
//               <Surface style={[styles.card, isEventCompleted(event.$id) && styles.cardCompleted]} elevation={0}>
...
//                         color="orange"
//                       />{" "}
//                       {/*//buraya 2:16 da tekrar bak videodan       */}
...
//         DATABASE_ID,
//         COLLECTION_ID,
//         [Query.equal("user_id", user.$id)] // Query'yi doğru yerde kullan
//       );
//       setEvents(response.documents as Events[]);
//     } catch (error) {
//       console.error("Error fetching events:", error);
...
//   const renderRightActions = () => (
//     <View style={styles.swipeActionRight}>
//       { isEventCompleted(eventId) ?(//Burada eventId parametresi eklenmeli
//         <Text style={{color:"#fff", fontWeight:"bold"}}>Completed</Text>
...
//         {events?.length === 0 ? (
//           <View style={styles.emptyState}>
//             <Text style={styles.emptyStateText}>No events found.</Text> {/* Typo düzeltildi */}
//           </View>
//         ) : (
//           events?.map((event, key) => (
//             <Swipeable
